...
private:
	vector<int> S;
	vector<int> F;
	vector<vector<table>> Q; //	1 вектор строки, 2 вектор столбцы
public:
...
bool intersection(vector<int> s, vector<int> f) { //пересечение
	for (int i = 0; i < s.size(); i++)
		for (int j = 0; j < f.size(); j++)
			if (s[i] == f[j]) return true;
	return false;
}
vector<int> union_q(vector<int> q, char k, Automat* M) { //	объединение
	vector<int> prev;
	vector<int> res;
...
pair<bool, int> maxString(Automat* M, string w, int p) { //	основная функция
	bool flag;
	int r;
...
int main() {
	...
	int n, g, kol, per,st; //число состояний
	char p;
	cout << "Введите число состояний ";
	cin >> n;
	vector<int> S, F; //  множество начальных и множество заключительных состояний
	vector<char> alph;
